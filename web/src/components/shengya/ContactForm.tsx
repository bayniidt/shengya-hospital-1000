  ...
  return <>
    <form className="weui-form sy-form" onSubmit={submit}>
      <div className="weui-form__control-area"><div className="weui-cells__group weui-cells__group_form"><div className="weui-cells">
        ...
        <label className="weui-cell weui-cell_active"><span className="weui-cell__hd"><span className="weui-label">{text('联系电话', 'Phone number')}</span></span><span className="weui-cell__bd"><input className="weui-input" required name="phone" type="tel" inputMode="tel" pattern="[0-9+() -]{6,20}" placeholder={text('请输入手机号', 'Enter your phone number')} /></span></label>
      ...
      <label className="weui-agree sy-check"><input required type="checkbox" name="privacyConsent" className="weui-agree__checkbox" /><span className="weui-agree__text">{text('我已阅读并同意', 'I have read and agree to the')} <a href="/privacy">{text('隐私说明', 'privacy notice')}</a></span></label>
